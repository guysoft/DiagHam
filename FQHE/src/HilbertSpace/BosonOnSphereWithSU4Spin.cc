...
void BosonOnSphereWithSU4Spin::TransformOneBodyBasis(ComplexVector& initialState, ComplexVector& targetState, ComplexMatrix* oneBodyBasis)
{
  ...
  for (int i = 0; i < this->HilbertSpaceDimension; ++i)
	...
	{
	  for (int l = 0; l < this->TemporaryStateDownMinus[j]; ++l)
	    {
	      TmpMomentumIndices[TmpIndex] = j;
	      TmpSU4Indices[TmpIndex] = 3;
	      ++TmpIndex;	      
	    }
	  for (int l = 0; l < this->TemporaryStateDownPlus[j]; ++l)
	    {
	      TmpMomentumIndices[TmpIndex] = j;
	      TmpSU4Indices[TmpIndex] = 2;
	      ++TmpIndex;	      
	    }
	  for (int l = 0; l < this->TemporaryStateUpMinus[j]; ++l)
	    {
	      TmpMomentumIndices[TmpIndex] = j;
	      TmpSU4Indices[TmpIndex] = 1;
	      ++TmpIndex;	      
	    }
	  for (int l = 0; l < this->TemporaryStateUpPlus[j]; ++l)
	    {
	      TmpMomentumIndices[TmpIndex] = j;
...
ComplexMatrix BosonOnSphereWithSU4Spin::TransformationMatrixSU4ToU1(BosonOnSphereShort* targetSpace, int type)
{
  ...
  unsigned long* TmpStateDescriptionOther3;
  switch (type)
    {
    case 0:
      {
