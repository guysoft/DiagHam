																		     ...
																		     char* precalculationFileName)
{
  ...
  while ((this->NbrPseudopotentialsUpUp > 0) && (this->PseudoPotentials[0][this->NbrPseudopotentialsUpUp - 1] == 0.0))
    {
  ...
  if (this->NbrPseudopotentialsUpUp > 0)
	...
	{
	  this->PseudopotentialsUpUp[i] = this->PseudoPotentials[0][i];
	}
    } 
  else
    {
  ...
  while ((this->NbrPseudopotentialsDownDown > 0) && (this->PseudoPotentials[1][this->NbrPseudopotentialsDownDown - 1] == 0.0))
    {
  ...
  if (this->NbrPseudopotentialsDownDown > 0)
	...
	{
	  this->PseudopotentialsDownDown[i] = this->PseudoPotentials[1][i];
	}
    } 
  else
    {
  ...
  while ((this->NbrPseudopotentialsUpDown > 0) && (this->PseudoPotentials[2][this->NbrPseudopotentialsUpDown - 1] == 0.0))
    {
  ...
  if (this->NbrPseudopotentialsUpDown > 0)
	...
	{
	  this->PseudopotentialsUpDown[i] = this->PseudoPotentials[2][i];
	}
    } 
  else
    {
...
double ParticleOnCylinderWithSpinTimeReversalSymmetricGenericHamiltonianAndPairing::EvaluateInteractionCoefficient(int m1, int m2, int m3, int m4, int nbrPseudopotentials, double* pseudopotentials)
{
  ...
  Coefficient += pseudopotentials[1] * (((double) ((m1 - m2) * (m1 - m2))) 
					+ ((double) ((m3 - m4) * (m3 - m4)))) * exp (-0.25 * Kappa2Factor * (((double) ((m1 - m2) * (m1 - m2))) 
													     + ((double) ((m3 - m4) * (m3 - m4))))) / sqrt(2.0 * M_PI);
  return Coefficient;
}
