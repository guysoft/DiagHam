...
void ParticleOnSphereWithSU4SpinS2Hamiltonian::EvaluateInteractionFactors()
{
  ...
  double Factor = 2.0 * this->S2Factor;
  ...
  this->OneBodyInteractionFactorsupup[0] = this->S2Factor * Coefficients(0, 1);
  this->OneBodyInteractionFactorsdowndown[0] = this->S2Factor * Coefficients(0, 1);
  for (int i = 1; i < this->LzMax; ++i)
    {
      this->OneBodyInteractionFactorsupup[i] = this->S2Factor * (Coefficients(i, i + 1) + Coefficients(i - 1, i));
      this->OneBodyInteractionFactorsdowndown[i] = this->S2Factor * (Coefficients(i, i + 1) + Coefficients(i - 1, i));
    }	  
  this->OneBodyInteractionFactorsupup[this->LzMax] = this->S2Factor * Coefficients(this->LzMax - 1, this->LzMax);
  this->OneBodyInteractionFactorsdowndown[this->LzMax] = this->S2Factor * Coefficients(this->LzMax - 1, this->LzMax);  
