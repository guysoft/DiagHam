	  ...
	  if (Manager.GetBoolean("decoupled") == false)
	      ...
	      for (int Sz = -NbrParticles; Sz <= NbrParticles; Sz += 2)
		{
		  ...
		  if (Manager.GetBoolean("flat-band") == true)
		    {
		      if (Manager.GetDouble("mu-s") == 0.0)
			sprintf (EigenstateOutputFile, "fermions_twoband_quantumspinhall_checkerboardlattice_n_%d_x_%d_y_%d_v_%f_w_%f_t1_%f_t2_%f_gx_%f_gy_%f_kx_%d_ky_%d_sz_%d", NbrParticles, NbrSitesX, NbrSitesY, Manager.GetDouble("v-potential"), Manager.GetDouble("w-potential"), 
				 Manager.GetDouble("t1"), Manager.GetDouble("t2"), Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"), i, j, Sz);
		      else
			sprintf (EigenstateOutputFile, "fermions_twoband_quantumspinhall_checkerboardlattice_n_%d_x_%d_y_%d_v_%f_w_%f_t1_%f_t2_%f_gx_%f_gy_%f_mus_%f_kx_%d_ky_%d_sz_%d", NbrParticles, NbrSitesX, NbrSitesY, Manager.GetDouble("v-potential"), Manager.GetDouble("w-potential"), 
				 Manager.GetDouble("t1"), Manager.GetDouble("t2"), Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"), Manager.GetDouble("mu-s"), i, j, Sz);
		    }
		  else
		    {
		      if (Manager.GetDouble("mu-s") == 0.0)
			sprintf (EigenstateOutputFile, "fermions_twoband_quantumspinhall_checkerboardlattice_n_%d_x_%d_y_%d_u_%f_v_%f_w_%f_t1_%f_t2_%f_gx_%f_gy_%f_kx_%d_ky_%d_sz_%d", NbrParticles, NbrSitesX, NbrSitesY, Manager.GetDouble("u-potential"), 
				 Manager.GetDouble("v-potential"), Manager.GetDouble("w-potential"), Manager.GetDouble("t1"), Manager.GetDouble("t2"), Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"), i, j, Sz);
		      else
			sprintf (EigenstateOutputFile, "fermions_twoband_quantumspinhall_checkerboardlattice_n_%d_x_%d_y_%d_u_%f_v_%f_w_%f_t1_%f_t2_%f_gx_%f_gy_%f_mus_%f_kx_%d_ky_%d_sz_%d", NbrParticles, NbrSitesX, NbrSitesY, Manager.GetDouble("u-potential"), 
				 Manager.GetDouble("v-potential"), Manager.GetDouble("w-potential"), Manager.GetDouble("t1"), Manager.GetDouble("t2"), Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"), Manager.GetDouble("mu-s"), i, j, Sz);
		    }
		  GenericComplexMainTask Task(&Manager, Hamiltonian->GetHilbertSpace(), &Lanczos, Hamiltonian, ContentPrefix, CommentLine, 0.0,  EigenvalueOutputFile, FirstRunFlag, EigenstateOutputFile);
		  FirstRunFlag = false;
		  MainTaskOperation TaskOperation (&Task);
