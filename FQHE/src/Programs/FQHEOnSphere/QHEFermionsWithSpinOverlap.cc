...
int main(int argc, char** argv)
{
  ...
  (*SystemGroup) += new SingleIntegerOption  ('m', "memory", "amount of memory that can be allocated for fast multiplication (in Mbytes)", 0);
  ...
  FermionOnSphereWithSpin *Space;
      ...
      if (LzMax <= 31)
        {
          Space = new FermionOnSphereWithSpin(NbrFermions, 0 /* assume L=0 for GS */, LzMax, SzTotal, MemorySpace);
        }
      else
	{
...
//      cout << ValueExact  << endl; 
  ...
  return 0;
  // calculate overlap with trial wave function:
