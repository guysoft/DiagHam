...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < NbrSpaces; ++i)
    {
      ...
      if (FQHEOnTorusWithSU3SpinFindSystemInfoFromVectorFileName(GroundStateFiles[i],
								 NbrParticles, KyMax, TotalKy[i], TotalTz, TotalY, Statistics) == false)
	{
  ...
  for (int i = 0; i < NbrSpaces; ++i)
	...
	{
	  Spaces[i] = new BosonOnTorusWithSU3Spin (NbrParticles, TotalTz, TotalY, KyMax, TotalKy[i]);
	}
  ...
  int NbrN1 = (2 * NbrParticles) + TotalY + (3 * TotalTz);
  int NbrN2 = (2 * NbrParticles) + TotalY - (3 * TotalTz);
  int NbrN3 = NbrParticles - TotalY;
  NbrN1 /= 6;
  NbrN2 /= 6;
  NbrN3 /= 3;
  ...
  for (; SubsystemNbrParticles <= MaxSubsystemNbrParticles; ++SubsystemNbrParticles)
    {
      ...
      for (int SubsystemNbrN1 = 0; SubsystemNbrN1 <= NbrN1; ++SubsystemNbrN1)
	for (int SubsystemNbrN2 = 0; SubsystemNbrN2 <= NbrN2; ++SubsystemNbrN2)
	  {
		...
		for (; SubsystemTotalKy <= SubsystemMaxTotalKy; ++SubsystemTotalKy)
		  {
		    ...
		    RealSymmetricMatrix PartialDensityMatrix = Spaces[0]->EvaluatePartialDensityMatrixParticlePartition(SubsystemNbrParticles, SubsystemTotalKy, 
															SubsystemNbrN1, SubsystemNbrN2, SubsystemNbrN3, GroundStates[0], Architecture.GetArchitecture());
			...
			RealSymmetricMatrix TmpMatrix = Spaces[i]->EvaluatePartialDensityMatrixParticlePartition(SubsystemNbrParticles, SubsystemTotalKy, 
														 SubsystemNbrN1, SubsystemNbrN2, SubsystemNbrN3, GroundStates[i], Architecture.GetArchitecture());
