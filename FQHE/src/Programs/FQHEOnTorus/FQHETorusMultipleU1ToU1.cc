  ...
  if (HaveComplex == false)
      ...
      else
	{
	  ...
	  if (GroupNeighbouringOrbitalsFlag == false)
		...
		{
		  OutputFileName = new char [512];
		  sprintf (OutputFileName, "bosons_torus_kysym_ky_%d_xsymmetrized_n_%d_2s_%d_ky_%d.0.vec", TotalKy1, NbrParticles1, NbrFluxQuanta, TotalKy);
		}
	      ...
	      if (OutputState.Norm() != 0.0)
		{
		  if (Manager.GetString("output-file") != 0)
		    ...
		    {
		      OutputFileName = new char [512];
		      sprintf (OutputFileName, "bosons_torus_kysym_ky_%d_ysymmetrized_n_%d_2s_%d_ky_%d.0.vec", TotalKy1, NbrParticles1, NbrFluxQuanta, TotalKy);
		    }
		  if (OutputState.WriteVector(OutputFileName) == false)
		    {
		      cout << "error while writing output state " << OutputFileName << endl;
      ...
      else
	{
	  ...
	  if (Manager.GetString("output-file") != 0)
	  ...
	  else
	    {
	      ...
	      if (Manager.GetBoolean("sym-y") == false)
		{
		  sprintf (OutputFileName, "bosons_torus_kysym_ky_%d_xsymmetrized_n_%d_2s_%d", TotalKy1, NbrParticles1, NbrFluxQuanta);
		}
	      else
		{
		  sprintf (OutputFileName, "bosons_torus_kysym_ky_%d_ysymmetrized_n_%d_2s_%d", TotalKy1, NbrParticles1, NbrFluxQuanta);
		}
	    }
