...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < NbrSpaces; ++i)
    {
      ...
      if (FQHEOnTorusWithSU4SpinFindSystemInfoFromVectorFileName(GroundStateFiles[i],
								 NbrParticles, KyMax, TotalKy[i], TotalSz, TotalIz, TotalPz, Statistics) == false)
	{
  ...
  for (int i = 0; i < NbrSpaces; ++i)
	...
	{
	  Spaces[i] = new BosonOnTorusWithSU4Spin (NbrParticles, TotalSz, TotalIz, TotalPz, KyMax, TotalKy[i]);
	}
  ...
  int NbrNUpPlus = (NbrParticles + TotalSz + TotalIz + TotalPz);
  int NbrNUpMinus = (NbrParticles + TotalSz - TotalIz - TotalPz);
  int NbrNDownPlus = (NbrParticles - TotalSz + TotalIz - TotalPz);
  int NbrNDownMinus = (NbrParticles - TotalSz - TotalIz + TotalPz);
  NbrNUpPlus >>= 2;
  NbrNUpMinus >>= 2;
  NbrNDownPlus >>= 2;
  NbrNDownMinus >>= 2;
  ...
  for (; SubsystemNbrParticles <= MaxSubsystemNbrParticles; ++SubsystemNbrParticles)
    {
      ...
      for (int SubsystemNbrNUpPlus = 0; SubsystemNbrNUpPlus <= NbrNUpPlus; ++SubsystemNbrNUpPlus)
	for (int SubsystemNbrNUpMinus = 0; SubsystemNbrNUpMinus <= NbrNUpMinus; ++SubsystemNbrNUpMinus)
	  for (int SubsystemNbrNDownPlus = 0; SubsystemNbrNDownPlus <= NbrNDownPlus; ++SubsystemNbrNDownPlus)
	    {
	      int SubsystemNbrNDownMinus = SubsystemNbrParticles - SubsystemNbrNUpPlus - SubsystemNbrNUpMinus - SubsystemNbrNDownPlus;
	      if (SubsystemNbrNDownMinus >= 0)
		{
		  ...
		  for (; SubsystemTotalKy <= SubsystemMaxTotalKy; ++SubsystemTotalKy)
		    {
		      ...
		      RealSymmetricMatrix PartialDensityMatrix = Spaces[0]->EvaluatePartialDensityMatrixParticlePartition(SubsystemNbrParticles, SubsystemTotalKy, 
															  SubsystemNbrNUpPlus, SubsystemNbrNUpMinus, SubsystemNbrNDownPlus, SubsystemNbrNDownMinus, GroundStates[0], Architecture.GetArchitecture());
