...
{  
  ...
  double tmp = -2.0; double tmp1 = -3.3;
  //potential->SetPotential(0, 0, 0, tmp);
  potential->SetPotential(M / 2, N / 2, 0, tmp);
  // define Hilbert space
  XYReflexionSymmetricPeriodic3DOneParticle GeneralSpace(M / 2, N / 2, H, -H / 2);
