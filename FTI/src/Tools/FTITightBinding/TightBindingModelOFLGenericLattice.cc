...
TightBindingModelOFLGenericLattice::TightBindingModelOFLGenericLattice(int nbrPoints1, int nbrPoints2, double gamma1, double gamma2, AbstractArchitecture* architecture, double latticeDepth, bool storeOneBodyMatrices)
{
  ...
  this->CutOffMomentum = this->Options->GetDouble("cut-off-momentum");
  this->CutOffType = this->Options->GetInteger("cut-off-type");
  int CutOffIndex=0;
  switch(this->CutOffType)
    {
    case TightBindingModelOFLGenericLattice::Rectangular:
      CutOffIndex=(int)nearbyint(this->CutOffMomentum);
      break;
    case TightBindingModelOFLGenericLattice::Periodic:
      CutOffIndex=(int)nearbyint(this->CutOffMomentum);
      break;
    case TightBindingModelOFLGenericLattice::AbsoluteValue:
      CutOffIndex=0;
      break;
    default:
      cout << "Error: --cut-off-type must be one of 0,1,2"<<endl;
      std::exit(1);
      break;
    }

  // this->DeltaK1, this->DeltaK2 defined below as a function of the reciprocal lattice vectors
	  ...
	  if (NbrLabels!=NbrExtParameters)
	    {
	      ...
	      for (int i=0; i<NbrLabels; ++i) delete [] ParameterLabels[i];
	      delete [] ParameterLabels;
	    }
	  char *NewDescriptor = new char[strlen(this->Descriptor)+4+15*NbrExtParameters];
	  int offset=0;
	  offset+=sprintf(NewDescriptor,"%s_ep_%g",Descriptor,ExtParameters[0]);
	  for (int i=1; i<NbrExtParameters; ++i)
	    {
	      offset+=sprintf(NewDescriptor+offset,"_%g",ExtParameters[i]);
	    }
  ...
  switch(this->CutOffType)
    ...
    case TightBindingModelOFLGenericLattice::Rectangular:
      ...
      this->NMax1 = CutOffIndex;
      this->NMax2 = CutOffIndex;
      break;
  ...
  this->NbrBands = (2*this->NMax1+1)*(2*this->NMax2+1) * NbrSubLattices;
#ifdef __LAPACK__
  this->NbrBandsToCalculate = this->Options->GetInteger("OFL-nbr-bands");
  if (this->NbrBandsToCalculate<=0)
    this->NbrBandsToCalculate = this->NbrBands;
#else
  this->NbrBandsToCalculate = this->NbrBands;
#endif
  ...
  this->EnergyBandStructure = new double*[this->NbrBandsToCalculate];
  for (int i = 0; i < this->NbrBandsToCalculate; ++i)
    {
...
TightBindingModelOFLGenericLattice::~TightBindingModelOFLGenericLattice()
{
  ...
  delete [] this->DeltaG1;
  delete [] this->DeltaG2;
  delete [] this->InitialSublattice;
  delete [] this->FinalSublattice;
  delete [] this->JumpAmplitudes;
...
void TightBindingModelOFLGenericLattice::AddOptionGroup(OptionManager* manager)
{
  ...
  (*LatticeGroup) += new SingleDoubleOption  ('\n', "cut-off-momentum", "value of cut-off according to cut-off-type", 20.0);
  (*LatticeGroup) += new SingleIntegerOption  ('\n', "cut-off-type", "0=rectangular, 1=periodic, 2=absolute value",0);
  (*LatticeGroup) += new MultipleDoubleOption ('\n', "external-parameters", "values of external parameters for lattice definition", ',');
  (*LatticeGroup) += new BooleanOption ('\n', "output-newlines", "energies of different band printed on separate lines in output");
  (*LatticeGroup) += new SingleIntegerOption  ('\n', "OFL-nbr-bands", "number of bands to calculate (and print in output files) (-1=all)",-1);
}
  ...
  for (int k1 = 0; k1 < this->NbrPoints1 ; ++k1)
      ...
      for (int k2 = 0; k2 < this->NbrPoints2; ++k2)
	{
	  ...
	  if ((Index >= minStateIndex) && (Index < MaxStateIndex))
...
#ifdef DEBUG_OUTPUT
			  cout << "KE (ii={"<<q1-NMax1<<","<<q2-NMax2<<") = "<<InitialNorm*InitialNorm/(M_PI*M_PI)<<" on spin "<<i<<endl;
#endif
			}
		      // hopping terms:
...
#ifdef DEBUG_OUTPUT
				  cout << "OD (ii={"<<q1-NMax1<<","<<q2-NMax2<<"}, jj={"<<q1+DeltaG1[j]-NMax1<<","<<q2+DeltaG2[j]-NMax2<<") = "<<this->LatticeDepth*this->JumpAmplitudes[j]<< " on "<<InitialSublattice[j]<<"->"<<FinalSublattice[j]<<endl;
#endif
				}
#ifdef DEBUG_OUTPUT
			      else
				cout << "omitted OD (ii={"<<q1-NMax1<<","<<q2-NMax2<<"}, jj={"<<q1+DeltaG1[j]-NMax1<<","<<q2+DeltaG2[j]-NMax2<<") = "<<this->LatticeDepth*this->JumpAmplitudes[j]<<" on "<<InitialSublattice[j]<<"->"<<FinalSublattice[j]<<endl;
#endif
			    }
			}
		    }
		...
		{
		  // xxx - we are potentially saving on space in TmpMatrix here - this may cause problems in other parts of the TightBindingModel routines
		  ComplexMatrix TmpMatrix(this->NbrBands, this->NbrBandsToCalculate, true);
		  TmpMatrix.SetToIdentity();
		  RealDiagonalMatrix TmpDiag;
#ifdef __LAPACK__
		  if (this->NbrBandsToCalculate<this->NbrBands)
		    TmpOneBodyHamiltonian.LapackPartialDiagonalize(TmpDiag, TmpMatrix, 0, this->NbrBandsToCalculate-1, /* err=*/ 1e-8);
		  else
...
#ifdef __LAPACK__
		  if (this->NbrBandsToCalculate<this->NbrBands)
		    TmpOneBodyHamiltonian.LapackPartialDiagonalize(TmpDiag, 0, this->NbrBandsToCalculate-1, /* err=*/ 1e-8);
		  else
...
#endif
		  for (int i = 0; i < this->NbrBandsToCalculate; ++i)
		    cout << "Energy "<<i<<" = "<< TmpDiag(i, i)<<endl;
...
{
  ofstream File;
  bool NewLines = this->Options->GetBoolean("output-newlines");

  File.open(fileName);
  this->WriteASCIIHeader(File, '#');
  int LimitOut = this->Options->GetInteger("OFL-nbr-bands");;
  if ((LimitOut<0)||(this->NbrBands < LimitOut))
