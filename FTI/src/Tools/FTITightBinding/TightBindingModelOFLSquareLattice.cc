...
TightBindingModelOFLSquareLattice::TightBindingModelOFLSquareLattice(double laserStrength, double gammaX, double gammaY, AbstractArchitecture* architecture, bool storeOneBodyMatrices)
{
  this->NbrSiteX=
  this->NbrStep = 20;
  this->NbrPoints = 10;
		  ...
		  for (int t=0; t< this->NbrStep; t++)
		    {
		      ...
		      double MomentaX=K1-M_PI*this->NbrStep + 2*M_PI*t;
		      double MomentaY=K2-M_PI*this->NbrStep + 2*M_PI*p;
		      TmpOneBodyHamiltonian.SetMatrixElement(IntermediateIndex, IntermediateIndex,MomentaX*MomentaX+MomentaY*MomentaY);
		      int IntermediateIndex1=this->GetIntermediateLinearizedIndices(p+1, t+1, 0);
		      TmpOneBodyHamiltonian.SetMatrixElement(IntermediateIndex, IntermediateIndex1,LaserStrength/(8.0));
		      ...
		      IntermediateIndex1=this->GetIntermediateLinearizedIndices(p+1, t-1, 0);
		      TmpOneBodyHamiltonian.SetMatrixElement(IntermediateIndex, IntermediateIndex1,-1.0*LaserStrength/(8.0));
		      ...
		      TmpOneBodyHamiltonian.SetMatrixElement(IntermediateIndex, IntermediateIndex1,-I()*1.0*LaserStrength/(4.0));
