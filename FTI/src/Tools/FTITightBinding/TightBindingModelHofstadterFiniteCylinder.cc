	 ...
	 int   PosY = 0;
      ...
      for (int i=0; i<1; ++i)
      {
		  ...
		  if (InitialIndex>=FinalIndex)
		    TmpOneBodyHamiltonian.AddToMatrixElement(InitialIndex, FinalIndex, -1.0*TranslationPhase);
#ifdef DEBUG_OUTPUT
		  ...
		  for (PosY = 1; PosY < NbrSiteY - 1; ++PosY)
		      ...
		      for (int i=0; i<1; ++i)
			{
			  ...
			  int FinalIndex = this->EncodeSublatticeIndex(i+1, PosY, K1,TranslationPhase);
			  if (InitialIndex>=FinalIndex)
			    TmpOneBodyHamiltonian.AddToMatrixElement(InitialIndex, FinalIndex, -1.0*PhaseX*TranslationPhase);
#ifdef DEBUG_OUTPUT
			  cout << "H["<<InitialIndex<<"->"<<FinalIndex<<"]="<< -1.0*PhaseX*TranslationPhase<<endl;
#endif
			  FinalIndex = this->EncodeSublatticeIndex(i-1, PosY, K1, TranslationPhase);
			  if (InitialIndex>=FinalIndex)
			    TmpOneBodyHamiltonian.AddToMatrixElement(InitialIndex, FinalIndex, -1.0*Conj(PhaseX)*TranslationPhase);
#ifdef DEBUG_OUTPUT
