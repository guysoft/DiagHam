...
int main(int argc, char** argv)
{
  ...
  if (FQHEOnSquareLatticeFindSystemInfoFromVectorFileName(Manager.GetString("state"),
							  NbrParticles, NbrCellX, NbrCellY, MomentumX, MomentumY, Mass, Statistics) == false)
    {
      cout << "error while retrieving system parameters from file name " << Manager.GetString("state") << endl;
      return -1;
    }
  cout << "N=" << NbrParticles << " Nx=" << NbrCellX << " Ny=" << NbrCellY << " kx=" << MomentumX << " ky=" << MomentumY << " m=" << Mass << endl; 
  ...
  /// replace with TightBindingModel  
  // ParticleOnChernInsulatorSingleBandFunctionBasis Basis(NbrCellX, NbrCellY, Mass);

  TightBindingModelHofstadterSquare tightBindingModel( use the same parameters as in FCIHofstadterModel );

  int TotalNbrMomenta = NbrCellX * NbrCellY;
  Complex* Coefficients = new Complex[TotalNbrMomenta*NbrSubLattices];
  Complex* Coefficients2 = new Complex[TotalNbrMomenta*NbrSubLattices];
  for (int i = 0; i < TotalNbrMomenta; ++i)
    for (int alpha=0; alpha < NbrSubLattices; ++alpha)
      {
	tightBindingModel.GetFunctionValue(Position, Coefficients[NbrSubLattices*i+alpha], i, alpha, /*bandIndex*/ 0);  // need to define this function in analogy with ParticleOnChernInsulatorSingleBandFunctionBasis to yield u_n(k) exp(i k r)
      }
  int subX, subY;
  for (int Rjx = 0; Rjx < NbrCellX; ++Rjx)
    {
      for (int Rjy = 0; Rjy < NbrCellY; ++Rjy)
	{
	  for (int alphaJ=0; alphaJ < NbrSubLattices; ++alphaJ) // sublattice for r_j
	      ...
	      if (DensityFlag == false)
		{
		  for (int i = 0; i < TotalNbrMomenta; ++i)
		    {
		      tightBindingModel.GetFunctionValue(Position, Coefficients2[NbrSubLattices*i+alphaJ], i, alphaJ, /*bandIndex*/ 0);     
		    }
		...
		{	      
		  for (int i = 0; i < NbrPrecalculatedValues; ++i) 
		    {
		      Complex TmpValue2;
		      tightBindingModel.GetFunctionValue(Position, TmpValue2, i, alphaJ, /*bandIndex*/ 0);
