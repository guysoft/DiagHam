...
int main(int argc, char** argv)
{
  ...
  int IncNbrPointX =  NbrPointX+2;
  int IncNbrPointY =  NbrPointY+2;
 ...
 for (int Kx = 0; Kx < NbrSitesX; Kx++)
  ...
  Hamiltonian = new ParticleOnLatticeKagomeLatticeSingleBandHamiltonian(Space, NbrParticles, NbrSitesX, NbrSitesY,Manager.GetDouble("u-potential"), 0,&TightBindingModel1, true, Architecture.GetArchitecture(), Memory);
  sprintf (EigenstateFile, "%s_%s_gx_%g_gy_%g_kx_%d_ky_%d", StateFilePrefix,FileParameterString,    TrueGammaX,  TrueGammaY, TotalKx, TotalKy);
  sprintf (EigenvalueOutputFile, "%s_%s_gx_%g_gy_%g_kx_%d_ky_%d.dat", StateFilePrefix,FileParameterString,    TrueGammaX,  TrueGammaY, TotalKx, TotalKy);
  FirstRunFlag = true;
 ...
 for (int GammaX = 1; GammaX <   IncNbrPointX; GammaX++)
  {
    ...
    TrueGammaY =   ( ( (double) - 1) / ( (double) NbrPointY));
 ...
 for (int Kx = 0; Kx < NbrSitesX; Kx++)
  ...
  if (Architecture.GetArchitecture()->GetLocalMemory() > 0)
    Memory = Architecture.GetArchitecture()->GetLocalMemory();
  Architecture.GetArchitecture()->SetDimension(Space->GetHilbertSpaceDimension());	
  Hamiltonian = new ParticleOnLatticeKagomeLatticeSingleBandHamiltonian(Space, NbrParticles, NbrSitesX, NbrSitesY,Manager.GetDouble("u-potential"), 0, &TightBindingModel2, true, Architecture.GetArchitecture(), Memory);
  ...
  delete[] EigenvalueOutputFile;   
    ...
    for (int GammaY = 1; GammaY <  IncNbrPointY; GammaY++)
     {
       TrueGammaX =   ( ( (double)  - 1) / ( (double) NbrPointX));
...
for (int GammaX = 0; GammaX <   IncNbrPointX; GammaX++)
  {
    for (int GammaY = 0; GammaY <   IncNbrPointY ; GammaY++)
    {
     ...
     sprintf (EigenstateFile, "%s_%s_gx_%g_gy_%g_kx_%d_ky_%d.0.vec", StateFilePrefix,FileParameterString,    TrueGammaX,  TrueGammaY, TotalKx,TotalKy);
