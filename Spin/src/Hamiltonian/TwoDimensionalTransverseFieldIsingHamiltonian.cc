											     ...
											     double** hxFactor, double** hzFactor, bool periodicBoundaryConditions)
{
  ...
  for (int i = 0; i < this->NbrSpinX; i++)
	...
	{
	  this->HxFactors[i][j] = hxFactor[i][j];
  ...
  for (int i = firstComponent; i < LastComponent; ++i)
      ...
      for (int j = 0; j < this->NbrSpinX; j++)
	{
	  for (int k = 0; k < this->NbrSpinY; k++)
	    {
	      pos = this->Chain->Spi((k * NbrSpinX) + j, i, coef);
	      if (pos != dim)
		{
		  vDestination[pos] += coef * this->HxFactors[j][k] * TmpValue;
		}
	      pos = this->Chain->Smi((k * NbrSpinX) + j, i, coef);
	      if (pos != dim)
		{
		  vDestination[pos] += coef * this->HxFactors[j][k] * TmpValue;
		}
  ...
  for (int i = 0; i < dim; i++)
      ...
      for (int j = 0; j < this->NbrSpinX; j++)
	{
	  for (int k = 1; k < this->NbrSpinY; k++)
	    {
	      this->SzSzContributions[i] += this->Chain->SziSzj(((k - 1) * NbrSpinX) + j, (k * NbrSpinX) + j, i);
	    }
	}
      for (int k = 0; k < this->NbrSpinY; k++)
	{
	  for (int j = 1; j < this->NbrSpinX; j++)
	    {
	      this->SzSzContributions[i] += this->Chain->SziSzj((k * NbrSpinX) + j - 1, (k * NbrSpinX) + j, i);
	    }
	}
      if (this->PeriodicBoundaryConditions == true)
	{
	  for (int k = 0; k < this->NbrSpinY; k++)
	    {
	      this->SzSzContributions[i] += this->Chain->SziSzj((k * NbrSpinX), ((k + 1) * NbrSpinX) - 1, i);
	    }
	  for (int j = 0; j < this->NbrSpinX; j++)
	    {
	      this->SzSzContributions[i] += this->Chain->SziSzj(j, ((NbrSpinY - 1) * NbrSpinX) + j, i);
	    }
	}
    }
  for (int i = 0; i < dim; i++)
	  ...
	  for (int j = 0; j < this->NbrSpinX; j++)
	    {
	      for (int k = 0; k < this->NbrSpinY; k++)
		{
		  this->Chain->Szi((k * NbrSpinX) + j, i, Coefficient);
